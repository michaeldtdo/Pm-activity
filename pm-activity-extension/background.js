...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // New staging handlers
    ...
    return true;
  } else if (message.type === 'ANALYZE_ACTIVITY') {
    analyzeActivityAndCreateCard(message.reason, message.activityData, sender.tab).then(result => {
      sendResponse(result);
    });
    return true;
  } else if (message.type === 'GET_AUTO_CARD_SETTINGS') {
    chrome.storage.local.get('autoCardSettings').then(({ autoCardSettings }) => {
      sendResponse({ settings: autoCardSettings || getDefaultAutoCardSettings() });
    });
    return true;
  } else if (message.type === 'SAVE_AUTO_CARD_SETTINGS') {
    chrome.storage.local.set({ autoCardSettings: message.settings }).then(() => {
      sendResponse({ success: true });
    });
    return true;
  }
...
// Get default auto-card settings
function getDefaultAutoCardSettings() {
  return {
    enabled: true,
    analysisInterval: 10, // minutes
    minActivityThreshold: 3, // minimum number of activities to trigger analysis
    autoAnalyzeOnSwitch: true, // analyze when switching context
    autoAnalyzeOnIdle: true, // analyze when user goes idle
    includeVisibleContent: true // include page content in analysis (privacy setting)
  };
}

// Analyze activity and create card using AI
async function analyzeActivityAndCreateCard(reason, activityData, tab) {
  console.log('[PM Activity] Analyzing activity:', reason, activityData);

  // Check if auto-card generation is enabled
  const { autoCardSettings } = await chrome.storage.local.get('autoCardSettings');
  const settings = autoCardSettings || getDefaultAutoCardSettings();

  if (!settings.enabled) {
    console.log('[PM Activity] Auto-card generation disabled');
    return { success: false, reason: 'disabled' };
  }

  // Check if we have enough activity
  if (activityData.buffer.length < settings.minActivityThreshold) {
    console.log('[PM Activity] Not enough activity to analyze');
  ...
  const { claudeApiKey } = await chrome.storage.local.get('claudeApiKey');
  if (!claudeApiKey) {
    console.log('[PM Activity] No API key configured');
    return { success: false, reason: 'no_api_key' };
  }

  // Check for duplicate cards (avoid creating cards for same activity)
  const recentCards = stagingActivities.filter(a => {
    const timeDiff = Date.now() - new Date(a.timestamp).getTime();
    return timeDiff < 30 * 60 * 1000 && // Last 30 minutes
           a.data.autoGenerated &&
           a.data.hostname === activityData.hostname;
  });

  if (recentCards.length > 0) {
    console.log('[PM Activity] Recent card exists for this activity');
    return { success: false, reason: 'duplicate' };
  }

  try {
    // Format activity data for AI analysis
    const activitySummary = formatActivityForAnalysis(activityData);

    // Call Claude API to analyze activity
    const prompt = `You are helping a PM track their work activity. Analyze the following activity and create a structured activity card.

Activity Data:
${activitySummary}

Based on this activity, create a concise activity card in JSON format:
{
  "type": "meeting" | "doc_edit" | "decision" | "task" | "communication" | "research" | "other",
  "title": "Brief 6-10 word title describing what the user was doing",
  "summary": "1-2 sentence summary of the activity",
...
Be concise but informative. The title should be action-oriented.`;
    ...
    const activity = {
      id: generateId(),
      timestamp: new Date(activityData.buffer[0].timestamp).toISOString(),
      status: 'draft',
      manual: false,
      type: analysis.type,
      tool: detectToolFromHostname(activityData.hostname),
      data: {
        ...
        autoGenerated: true,
        autoGenReason: reason,
        hostname: activityData.hostname,
        duration: Math.floor(activityData.duration / 1000), // seconds
        activityCount: activityData.buffer.length
      },
      snippet: null,
      lastModified: new Date().toISOString(),
      tab_id: tab?.id,
    ...
    console.log('[PM Activity] Auto-generated card:', activity.data.title);
    ...
    console.error('[PM Activity] Auto-card generation error:', error);
    return {
      success: false,
      error: error.message
    };
...
// Format activity data for AI analysis
function formatActivityForAnalysis(activityData) {
  const { buffer, duration, hostname } = activityData;

  let summary = `Website: ${hostname}\n`;
  summary += `Duration: ${Math.floor(duration / 60000)} minutes\n`;
  summary += `Activity count: ${buffer.length}\n\n`;

  // Group activities by type
  const states = buffer.filter(a => a.reason);
  const interactions = buffer.filter(a => a.type && !a.reason);

  if (states.length > 0) {
    summary += 'Page Activity:\n';
    states.forEach(state => {
      const time = new Date(state.timestamp).toLocaleTimeString('en-US', {
        hour: 'numeric',
        minute: '2-digit'
      });
      summary += `  [${time}] ${state.reason}: ${state.title}\n`;
      if (state.detectedType) {
        summary += `    Type: ${state.detectedType}\n`;
      }
      if (state.visibleText) {
        summary += `    Content preview: ${state.visibleText.substring(0, 300)}...\n`;
      }
    });
  }
  ...
  if (interactions.length > 0) {
    ...
    interactions.slice(0, 10).forEach(interaction => {
      ...
      summary += `  [${time}] ${interaction.type}`;
      if (interaction.data?.text) {
        summary += `: ${interaction.data.text}`;
      }
      summary += '\n';
    });
  }
...
// Detect tool from hostname
function detectToolFromHostname(hostname) {
